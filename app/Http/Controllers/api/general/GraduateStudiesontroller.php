...
use App\Models\general\Graduatestudie_manager;
use App\Models\general\Graduatestudie_member;
use App\Models\general\GraduateStudies;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;

class GraduateStudiesontroller extends Controller
{
    use TraitResponse;
    public function uploadfile(Request $request)
    {
        ...
        $path = $request->file('file')->storeAs('public/general/uploads/GraduateStudies',$file);
        $files = GraduateStudies::create([
            'title'=>$request->title,
            'file'=>$file
        ]);
...
#################################################################################################################################
    public function UpdateFile(Request $request, $id)
    {
        ...
        $file = GraduateStudies::find($id);
        if(!$file) {
            return $this->apiResponse(null, 'Not Found', 404);
        }
        $fileupdate = $request->file('file')->getClientOriginalName();
        $path = $request->file('file')->storeAs('public/general/uploads/GraduateStudies',$fileupdate);
        $file->update([
            'title'=>$request->title,
...
public function uploadmembers(Request $request){
    ...
    $manager = Graduatestudie_member::create([
...
public function showmembers()
{
    ...
    $file = Graduatestudie_member::all();
...
public function updatemembers(Request $request,$id){
    ...
    $manager = Graduatestudie_member::find($id);
    if(!$manager){
        return $this->apiResponse(null,'Not Found',404);
    }
    $manager->update([
...
public function managerupload(Request $request){
    ...
    $path = $request->file('manager_biography')->storeAs('public/general/uploads/GraduateStudies',$file);
    $manger = Graduatestudie_manager::create([
...
public function showbiofraphy()
{
    ...
    $file = Graduatestudie_manager::all();
...
public function updatebiography(Request $request,$id){
    ...
    $manager = Graduatestudie_manager::find($id);
    if(!$manager){
        return $this->apiResponse(null,'Not Found',404);
    }
    $file = $request->file('manager_biography')->getClientOriginalName();
